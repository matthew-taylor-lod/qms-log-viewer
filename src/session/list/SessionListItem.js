...
function SessionListItem({session, setSelected}) {
    const algo = (session.patientUniverseName !== "null") ? session.patientUniverseName : session.algorithmId;
    const product = session.prePropHistory?.confInitialProduct;
    const outcome = (session.outcome)
        ? session.outcome.diagnosisOutcome.diagnosisStatus
        : "INCOMPLETE";
    const suitableResponse = session.outcome?.diagnosisOutcome.outcomeScriptData.suitable_response;

    const rowStyle = "SessionListItem " + outcome;
