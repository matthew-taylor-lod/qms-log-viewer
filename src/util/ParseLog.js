...
function ParseLog(text) {
    ...
    const output = individualSessions.map((items, i) => {
        ...
        session.sku = algoStartup.productId;
        session.algorithmId = algoStartup.algorithmId;
        session.patientUniverseName = algoStartup.patientUniverseName;
        session.prePropHistory = prePropHistory?.data;
...
function AlgoStartupLine(line) {
    ...
    item.algorithmId = line.match(/algorithmId=(\d*)/)[1];
    item.patientUniverseName = line.match(/patientUniverseName=(.*?)}/)[1];
